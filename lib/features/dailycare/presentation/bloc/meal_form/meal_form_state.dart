...
abstract class MealFormState with _$MealFormState {
  const factory MealFormState({
    ...
    @Default([]) final List<DropStringItem> mealtypes,
    @Default(DropdownValue.pure()) final DropdownValue mealTimeH,
    @Default(DropdownValue.pure()) final DropdownValue mealTimeM,
    @Default(DropdownStringValue.pure()) final DropdownStringValue mealType,
...
extension WalkValidation on MealFormState {
  bool get validationX {
    return Formz.validate([mealTimeH, mealTimeM, date, notes, media, mealType]);
  }
}
